...
public class AplicacionEmpleado extends JFrame { 
	...
	public static void main(String[] args) throws SQLException {
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					new AplicacionEmpleado(manager).setVisible(true);
				} catch (DAOException ex) {
					Logger.getLogger(AplicacionEmpleado.class.getName()).log(Level.SEVERE, null, ex);
	 ...
	 * @throws SQLException 
	 */
	public AplicacionEmpleado(DAOManager manager) throws DAOException, SQLException {
		setTitle("GESTIÓN EMPLEADOS ZARA");
...
//idtienda
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (table.getSelectedRow() >= 0) {
					...
					textonventas.setText(table.getValueAt(table.getSelectedRow(), 4).toString());
					textoEdad.setText(table.getValueAt(table.getSelectedRow(), 3).toString());
				}
			}
		});
		;
	...
	public boolean checkIfNotEmpty() {
		if (!textonventas.getText().isEmpty()
